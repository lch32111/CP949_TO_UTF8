...
std::unordered_map<uint32_t, uint16_t> utf8_to_cp949_map;

int32_t process_utf8_buffer(uint8_t* target_buffer, uint8_t* source_buffer, int32_t source_buffer_len);
...
int main(int argc, char* argv[])
{
	...
	for (int32_t i = 0; i < table_count; ++i)
	{
		...
		utf8_to_cp949_map[d.utf8] = d.cp949;
	}
	...
	int32_t target_processed_buffer_size = process_utf8_buffer(target_buffer, source_buffer, source_buffer_size);
...
int32_t process_utf8_buffer(uint8_t* target_buffer, uint8_t* source_buffer, int32_t source_buffer_len)
{
	...
	while (source_i < source_buffer_len)
	{
		...
		int32_t utf8_size = get_utf8_byte_size(data);

		if (utf8_size > 1)
		{
			utf8_v = 0;
			for (int32_t i = 0; i < utf8_size; ++i)
			{
				utf8_v = (utf8_v | (source_buffer[source_i + i] << (8 * i)));
			}
			cp949_v = utf8_to_cp949_map[utf8_v];

			target_buffer[target_i] = (cp949_v & 0xFF00) >> 8;
			target_buffer[target_i + 1] = (cp949_v & 0x00FF);
